...
bool		Arbitrator::canPlace(Player *p, Intersection *i){
  if (!p || !i)
    return (false);
...
Patern		*Arbitrator::winInter(Intersection *inter){
...
Patern		*Arbitrator::blockInter(Intersection *inter){
  if (!inter || !inter->getOwner())
    return (0);
...
bool		Arbitrator::canPlace(Intersection *inter){
  Intersection	*l = getInterPos(_field, inter, -50, 0);
  Intersection	*r = getInterPos(_field, inter, 50, 0);
  Intersection	*t = getInterPos(_field, inter, 0, -50);
  Intersection	*b = getInterPos(_field, inter, 0, 50);
...
Intersection	*Arbitrator::getInterPos(Field *f, Intersection *inter, int x, int y){
  if (!f || !inter)
    return (0);
